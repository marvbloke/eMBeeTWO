...
// Hardware I2C at 400kHz for real ATmega328P hardware
#include <Wire.h>
...
void setup()
{
  ...
  // Initialize Hardware Wire at 400kHz
  Wire.begin();
...
static void outchar(unsigned char c) {
  ...
  #if SIMULATOR_BUILD
      // Wokwi can't scroll, so when we hit the bottom row (7), clear the whole screen
      if (oled.row() >= 7) {
        oled.clear();
        oled.setCursor(0, 0);
        return;
